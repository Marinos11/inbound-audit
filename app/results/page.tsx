...
const RESULTS_KEY = 'inbound-audit-results';
...
function ScoreRing({ score }: { score: number }) {
  ...
  const color =
    score >= 80 ? '#10b981' : score >= 60 ? '#c8a24a' : '#ef4444';
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <span className="text-xs text-red-400">
              {leak.score}/{leak.maxScore} Punkte
            </span>
          </div>
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setResults(JSON.parse(raw) as QuizResults);
    } catch {
      /* ignore */
    }
    setMounted(true);
  ...
  if (!results) {
    return (
      <div className="min-h-screen bg-near-black flex flex-col items-center justify-center gap-6 px-5 text-center">
        <p className="text-white/50">Kein Audit-Ergebnis gefunden.</p>
        <Link href="/audit">
      ...
      <main className="max-w-2xl mx-auto px-5 py-12 space-y-16">
        ...
        <section className="space-y-6">
          ...
          <Card className="p-6 space-y-5">
            <CategoryBar
              label="Profil & Positionierung"
              value={categoryScores.profile}
              maxValue={30}
            />
            <CategoryBar
              label="Content & CTA"
            ...
            />
            <CategoryBar
              label="Distribution & Netzwerk"
              value={categoryScores.distribution}
              maxValue={20}
            />
            <CategoryBar
              label="Umsatz / Qualifizierung"
              value={categoryScores.revenue}
              maxValue={15}
            />
          </Card>
        </section>
        ...
        <section className="space-y-6">
          <div className="space-y-1">
            ...
            <p className="text-sm text-white/40">
              Sortiert nach Hebel – fang oben an.
            </p>
          </div>
